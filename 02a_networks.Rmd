...
# Data and subsets
```{r}
RESST_all <- read.csv("Created_data/RESST_merged_SG_2024-09-29.csv") # From "01a_data_processing"
...
RESST_ega_SA <- bootEGA(
  data = RESST_inv[RESST_inv$SA_group == "1", 1:21],
  corr = "auto",
  model = "glasso",
  algorithm = "walktrap",
  uni.method =  "louvain",
...
RESST_ega_no_SA <- bootEGA(
  data = RESST_inv[RESST_inv$SA_group == "0", 1:21],
  corr = "auto",
  model = "glasso",
  algorithm = "walktrap",
  uni.method =  "louvain",
...
plot(All_boot, "strength", order = "sample", labels = TRUE) # strength
plot(All_boot, "expectedInfluence", order = "sample", labels = TRUE) # expected influence
...
pdf("EBIC_network_clusters.pdf", width = 10, height = 10)
...
pdf("Output/EBIC_centrality_bridge.pdf", width = 10, height = 10)
plot(intbridge)
dev.off() 
...
pdf("Output/EBIC_boot_edges_2.pdf", width = 10, height = 10)
plot(All_boot, plot = "interval", split0 = TRUE, order = "sample", labels = FALSE)
dev.off()
